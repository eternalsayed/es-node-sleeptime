 ...
 * @param {Integer}  counts         Optional: Defines the counts after which
 *                                  the process should be stopped.
 *                                  default: indefinite
 */
function SleepTime() {
    if(!arguments.length) {
        throw new Error("Mandatory callback function is missing.");
    }
    const wakeUpCallback = arguments[0];
    const thresh = arguments[1];
    this.counts = arguments[2];
...
SleepTime.prototype.start = function() {
    ...
    self.timer = setInterval(self.tick, interval);
};
...
SleepTime.prototype.tick = function() {
    ...
    if(diff > self.thresh){
        if (!this.lasTickDidTrigger) {
            ...
            this.counter++;
            self.wakeUpCallback(diff, now, self.stop);//return 'stop' function for cases if user wanted to stop manually
            if(this.counts && this.counts >= this.counter) {
                this.stop();
            }
        }
    }
